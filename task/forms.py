...
class SendDocsForm(ModelForm):
    ...
    def clean_docs(self):
        docs = self.cleaned_data['docs']
        if not docs.name.lower().endswith(self.extensions):  # если не эксель расширение
            raise ValidationError(f'Расширение файла должно быть {self.extensions}')
        else:
            return docs
